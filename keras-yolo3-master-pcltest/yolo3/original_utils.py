...
def get_random_data_mixup(annotation_line, annotation_line_for_mixup, input_shape, random=True, alpha=1.5, max_boxes=20, jitter=.3, hue=.1, sat=1.5, val=1.5, proc_img=True):
    '''random preprocessing for real-time data augmentation'''
    line = annotation_line.split()
    image = Image.open(line[0])
    # import image for mixup
    line_for_mixup = annotation_line_for_mixup.split()
    image_for_mixup = Image.open(line_for_mixup[0])

    iw, ih = image.size
    # create a new picture
    iw_m,ih_m = image_for_mixup.size
    ...
    # mix box
    box_for_mixup = np.array([np.array(list(map(int,box.split(',')))) for box in line_for_mixup[1:]])
    ...
    return image_data, box_data, lambda_mixup, len_lambda
