            ...
            for d in range(len(dects)):
                ...
                if iouMax >= IOUThreshold:
                    if det[dects[d][0]][jmax] == 0:
                        ...
                        # added by pcl, if a prediction is not normal and gt is normal, throw it into "false positive"(here positive means pathological) 
                        if gt[j][1]=='normal' and dects[d][1]!='normal':
                            FPdistribution.append(float(dects[d][2]))
                        
                # - A detected "cat" is overlaped with a GT "cat" with IOU >= IOUThreshold.
                else:
                    FP[d] = 1  # count as false positive
                    # print("FP")
                    # added by pcl, if a prediction is not normal, throw it into "false positive"(here positive means pathological) 
                    if dects[d][1]!='normal':
                        FPdistribution.append(float(dects[d][2]))
