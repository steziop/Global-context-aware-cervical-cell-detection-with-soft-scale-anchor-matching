...
parser.add_argument(
    ...
    '--gtfolder',
    dest='gtFolder',
    default=os.path.join(currentPath, 'groundtruths_4000s'),
    metavar='',
...
# Get detected boxes
allBoundingBoxes, allClasses = getBoundingBoxes(
    detFolder, False, detFormat, detCoordType, allBoundingBoxes, allClasses, imgSize=imgSize, confidence_thresh = 0)
allClasses.sort()
...
validClasses = 0
all_total_TP = 0
all_totalPositives = 0
all_falsePositives = 0
all_FPdistribution = []
...
for metricsPerClass in detections:
    ...
    # added by pcl
    FPdistribution = metricsPerClass['FPdistribution']

    if totalPositives > 0:
        # added by pcl
        # print(cl)
        if cl!='normal':
            all_total_TP += total_TP
            all_totalPositives += totalPositives
            all_falsePositives += total_FP
            all_FPdistribution.extend(FPdistribution)
...
'''
# added by pcl
f2 = open(os.path.join('C:/Users/pcl/Desktop/Object-Detection-Metrics-master', 'FPdistribution.txt'), 'a+')
f2.write(str(all_FPdistribution)+'\n')
f2.close()
'''
mAP = acc_AP / validClasses
mAP_str = "{0:.2f}%".format(mAP * 100)
print('mAP: %s' % mAP_str)
print('recall: ',all_total_TP/all_totalPositives)
print('tp: ', all_total_TP)
print("fp: ", all_falsePositives)
print("jia yang:", len(all_FPdistribution))
f.write('\n\n\nmAP: %s' % mAP_str)
f.close()
